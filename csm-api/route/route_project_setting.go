...
func ProjectSettingRoute(safeDB *sqlx.DB, r *store.Repository) chi.Router {
	...
	router.Post("/man-hours", projectSettingHandler.MergeManHour)           // 프로젝트 공수 정보 추가 및 수정
	router.Delete("/man-hours/{mhno}", projectSettingHandler.DeleteManHour) // 프로젝트 공수정보 삭제
	return router
}
